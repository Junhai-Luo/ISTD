...
#                 conv_weight_rd = torch.cuda.FloatTensor(conv_shape[0], conv_shape[1], 5 * 5).fill_(0).to(conv_weight.device)
#             else:
...
class DEConvio(nn.Module):
    def __init__(self, indim, outdim):
        super(DEConvio, self).__init__()
...
class DEConvNoRep(nn.Module):
    def __init__(self, indim, outdim):
        super(DEConvNoRep, self).__init__()
        self.conv1_1 = Conv2d_cd(indim, outdim, 3, bias=True)
        self.conv1_2 = Conv2d_hd(indim, outdim, 3, bias=True)
        self.conv1_3 = Conv2d_vd(indim, outdim, 3, bias=True)
        self.conv1_4 = Conv2d_ad(indim, outdim, 3, bias=True)
    ...
    def forward(self, x):
        ...
        res1 = nn.functional.conv2d(input=x, weight=w1, bias=b1, stride=1, padding=1, groups=1)
        res2 = nn.functional.conv2d(input=x, weight=w2, bias=b2, stride=1, padding=1, groups=1)
        res3 = nn.functional.conv2d(input=x, weight=w3, bias=b3, stride=1, padding=1, groups=1)
        res4 = nn.functional.conv2d(input=x, weight=w4, bias=b4, stride=1, padding=1, groups=1)
        res5 = nn.functional.conv2d(input=x, weight=w5, bias=b5, stride=1, padding=1, groups=1)
